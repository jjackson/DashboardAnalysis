...
class SupersetExtractor:
    ...
    def __init__(self, superset_url: Optional[str] = None, username: Optional[str] = None, password: Optional[str] = None):
        """
        ...
        if not all([self.superset_url, self.username, self.password]):
            missing = []
            if not self.superset_url: missing.append('SUPERSET_URL')
            if not self.username: missing.append('SUPERSET_USERNAME')
            if not self.password: missing.append('SUPERSET_PASSWORD')
        ...
        if csrf_response.status_code == 200:
            ...
            if self.csrf_token:
                self.session.headers.update({
                    ...
                    'Referer': f"{self.superset_url}/sqllab"
                })
    ...
    def export_query_to_csv(self, sql_query: str, output_filename: Optional[str] = None, 
